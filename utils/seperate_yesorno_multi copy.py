...
<head>
    ...
    <title>AZ 400 Dumps - Yes or No (Multi)</title>
...
for ele in case_study_questions:
    ...
    for pele in ptag:
        if pele.find(text=re.compile(r'select Yes if the statement is true. Otherwise, select No.')) is not None:
...
with open(f'yesorno/multiple/index.html', 'w',encoding='utf8') as f1:
    f1.write(finalhtml)
