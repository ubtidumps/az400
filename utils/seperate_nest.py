...
with open('yesorno/index.html', 'r') as f:
    content = f.read()
...
data = [
    'You want to implement an Azure AD conditional access policy', # 3
    'To achieve this, the Per Enabled User setting must be set for the usage model', # 3
    'You have a server named DirSync1 that is configured as a DirSync server', # 3
    'You want to review the ARM template that was used by Jon Ross', # 3
    'You have to make sure that a connection to VirtualNetworkB can be established from the Windows 10 workstation', # 3
    'You need to configure an Azure internal load balancer as a listener for the availability group', # 3
    'You need to create new user accounts in external.contoso.onmicrosoft.com', # 4
    'You need to provide the Developers group with the ability to create Azure logic apps in the Dev resource group',# 4
    'You need to ensure that an Azure Active Directory \(Azure AD\) user named Admin1 is assigned the required role to enable Traffic Analytics for an Azure subscription',# 4
    'You have an Azure virtual machine named VM1 that runs Windows Server 2016',# 5
    'You need to create a new network interface named NIC2 for VM1',# 3
    'You need to deploy a YAML file to AKS1',# 3
    'You need to move VM1 to a different host immediately',# 4
    'You need to connect VM1 to VNET2',# 4
    'You need to view the date and time when the resources were created in RG1',# 4
    'You need to be able to establish Remote Desktop connections from the internet to VM1',# 3
    'You need to ensure that you can establish a point-to-site VPN connection to VNet1 from Computer2',# 4
    'You need to ensure that when an NSG is created, it automatically blocks TCP port 8080 between the virtual networks',# 4
    'You need to ensure that you can add VM1 and VM2 to the backend pool of LB1',# 4
    'You verify that the Load Balancer rules are configured correctly',# 5
    'You need to inspect all the network traffic from VM1 to VM2 for a period of three hours'# 4
]
...
'''
    for ele in case_study_questions:
        ...
        for pele in ptag:
            if pele.find(text=re.compile(q)) is not None:
    ...
    '''
    os.mkdir(f'yesorno/{i}')    
    with open(f'yesorno/{i}/index.html','w') as f1:
        f1.write(finalhtml)
